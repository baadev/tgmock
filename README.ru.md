...
Когда бот работает с реальным Telegram API, тесты часто становятся хрупкими и медленными.
`tgmock` поднимает локальный HTTP mock Bot API, чтобы вы могли:

- отправлять боту текст и фото,
- нажимать inline-кнопки,
- смотреть ответы и логи,
- собирать кастомные события,
- запускать одинаковый флоу через Codex MCP и `pytest`.
